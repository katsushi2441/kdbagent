...
function kdba_api(array $fields)
{
    ...
    if ($body === false) {
        return array(false, json_encode(array('ok' => false, 'error' => kdba_mcp_t('サーバーに繋がりません: ') . $err), JSON_UNESCAPED_UNICODE));
    }
    ...
    if ($code === 401) {
        return array(false, json_encode(array('ok' => false, 'error' => kdba_mcp_t('トークンが違います（サーバー側のKDBA_API_TOKENと一致していません）')), JSON_UNESCAPED_UNICODE));
    }
    ...
    array(
        ...
        'inputSchema' => array('type' => 'object',
            'properties' => array(
                ...
                'where'  => array('type' => 'object', 'description' => kdba_mcp_t('列名と値の完全一致条件（任意）')),
...
$TOOLS_WRITE = array(
    array(
        ...
        'inputSchema' => array('type' => 'object',
            'properties' => array(
                'table'  => array('type' => 'string', 'description' => kdba_mcp_t('表の名前')),
                'values' => array('type' => 'object', 'description' => kdba_mcp_t('列名と値の組')),
            ),
    ...
    switch ($method) {
        case 'initialize':
            kdba_result($id, array(
                ...
                'instructions'    => kdba_mcp_t('宣言された表・列・操作だけを扱うデータベース窓口です（サーバー上のkdbagentにHTTPSで接続）。まず kdb_tables で触れる範囲を確認してください。生SQLは実行できません。データを書き換える前に、利用者に内容を確認してください。'),
            ));
            break;
        case 'ping':
        ...
        case 'tools/call':
            ...
            if (!$known) {
                kdba_result($id, array('content' => array(array('type' => 'text', 'text' => kdba_mcp_t('このサーバーでは使えないツールです: ') . $name)), 'isError' => true));
                break;
            }
